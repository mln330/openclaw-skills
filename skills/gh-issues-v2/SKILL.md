...
IMPORTANT — Uses GitHub App authentication. The skill generates short-lived installation tokens from GH_APP_ID + GH_PRIVATE_KEY. Falls back to GH_TOKEN (PAT) if App auth unavailable.
...
## Phase 2 — Token Generation (GitHub App or PAT)

**NEW: Try GitHub App auth first, fall back to PAT**

**Step 2.1 — GitHub App Token Generation**

If GH_APP_ID and GH_PRIVATE_KEY_PATH are set, generate an installation token:

```
# Generate JWT from app private key
JWT_HEADER=$(echo -n '{"typ":"JWT","alg":"RS256"}' | base64 -w 0 | tr -d '=' | tr '+/' '-_')
NOW=$(date +%s)
JWT_PAYLOAD=$(echo -n "{\"iat\":$((NOW-60)),\"exp\":$((NOW+600)),\"iss\":\"$GH_APP_ID\"}" | base64 -w 0 | tr -d '=' | tr '+/' '-_')

# Sign with private key (requires openssl)
JWT_SIGNATURE=$(echo -n "$JWT_HEADER.$JWT_PAYLOAD" | openssl dgst -sha256 -sign "$GH_PRIVATE_KEY_PATH" | base64 -w 0 | tr -d '=' | tr '+/' '-_')

JWT="$JWT_HEADER.$JWT_PAYLOAD.$JWT_SIGNATURE"

# Get installation ID (search by repo owner)
SOURCE_REPO_OWNER=$(echo "$SOURCE_REPO" | cut -d'/' -f1)
INSTALLATION_ID=$(curl -s -H "Authorization: Bearer $JWT" -H "Accept: application/vnd.github+json" \
  "https://api.github.com/app/installations" | jq -r ".[] | select(.account.login==\"$SOURCE_REPO_OWNER\") | .id")

# Get installation token
GH_TOKEN=$(curl -s -X POST -H "Authorization: Bearer $JWT" -H "Accept: application/vnd.github+json" \
...
if [ -n "$GH_TOKEN" ] && [ "$GH_TOKEN" != "null" ]; then
  ...
  echo "GitHub App auth failed, falling back to PAT"
fi
```

**Step 2.2 — Fallback to Personal Access Token (PAT)**

If GitHub App auth failed or env vars not set:
...
5. **Check for existing PRs:**
   ...
   If the response array is non-empty, check if PR is currently under review:
   
   ```
   PR_UPDATED_AT=$(echo "$EXISTING_PR" | jq -r '.[0].updated_at')
   PR_CREATED_AT=$(echo "$EXISTING_PR" | jq -r '.[0].created_at')
   AGE_HOURS=$(( ($(date +%s) - $(date -d "$PR_CREATED_AT" +%s)) / 3600 ))
   LAST_UPDATE_MIN=$(( ($(date +%s) - $(date -d "$PR_UPDATED_AT" +%s)) / 60 ))
   
   # If PR is new (< 2 hours old) AND recently updated (< 30 min ago), it's likely being reviewed
   # Don't abandon it — skip and let gh-prs finish reviewing
   if [ "$AGE_HOURS" -lt 2 ] && [ "$LAST_UPDATE_MIN" -lt 30 ]; then
     echo "PR #{N} is new and recently updated ($LAST_UPDATE_MIN min ago) — likely under review, waiting"
     DELAYED_ISSUES="$DELAYED_ISSUES $N"
     continue
   fi
   ```
   
   Only skip permanently if PR is older or stale:
...
- If `--model` was provided (legacy), include `model: "{MODEL}"` as fallback
...
**Spawn configuration per review fix sub-agent:**
